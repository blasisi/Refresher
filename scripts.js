...
for(let i =0; i < studentCount; i++){//then loop throught the array each time though printing the current index and the of the element at index in the array
...
for(const key in student){ //for every key or property (use key to represent the property and value in the object). in this case every key in object in pcuser.
...
function calcSum (enduser, addendUser){ // This calculate the sum of two numbers passed in as paramenter. 
...
function calTotalAverage(num){// I created  a function name calculateAverage and it take paramenter num . inside of the calTotalAverage function i declare a variable  named sumTotal and set it equal to zero
let sumTotal = 0;
for(let i = 0; i< num.length; i++){ //then looping throught the num array, eact time through increament by sum plus equal the values in the num arrray  increment index.
  sum += num[i];
  {
...
function addSum(add, subtract){//in this line  i declear a function called addSum, which take two argument

}

function  divSum (){ //also have another function and declare it as divSum, this function doesnt have no arguement.
